...
public final class Code {
	...
	public Code(String fichier) {
		...
		String[] tab = LectureFichier.lire(fichier).split("\n");

		System.out.println(afficherPseudoCode(tab));
		interpretation(tab);
	}

	private static String afficherPseudoCode(String[] tab) {
		String r = new String();
		for (int i = 0; i < tab.length; i++) {
			r += "|" + String.format("%3s", Integer.toString(i)) + " "
					 + String.format("%-135" +
							"s", tab[i]) + "|\n";
		}
